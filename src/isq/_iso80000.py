...
modules (e.g. [aerospace][isq.aerospace]) for performance reasons.

See: [isq._citations.SI][], [isq._citations.IUPAP1][],
[isq._citations.SP811][]
[isq._citations.CODATA2022][], [isq._citations.ISO_80000_2][],
[isq._citations.ISO_80000_3][], [isq._citations.ISO_80000_4][],
[isq._citations.ISO_80000_5][],
[isq._citations.IEC_80000_6][], [isq._citations.IEV][],
[isq._citations.ISO_80000_7][], [isq._citations.ISO_80000_8][],
[isq._citations.ISO_80000_9][], [isq._citations.ISO_80000_10][],
[isq._citations.ISO_80000_11][], [isq._citations.ISO_80000_12][],
[isq._citations.ISO_80000_13][], [isq._citations.MP_UNITS][].
...
"""Radian, a unit of plane [angle][isq.ANGLE]. Not to be confused with m m⁻¹."""
SR = Dimensionless("steradian")
"""Steradian, a unit of [solid angle][isq.SOLID_ANGLE].
Not to be confused with m² m⁻²."""
HZ = (S**-1)["frequency"].alias("hertz", allow_prefix=True)
"""Unit of [frequency][isq.FREQUENCY].
Shall only be used for periodic phenomena."""
M_PERS = M * S**-1
M_PERS2 = M * S**-2
N = (KG * M_PERS2).alias("newton", allow_prefix=True)
"""Newton, a unit of [force][isq.FORCE]."""
PA = (N * M**-2).alias("pascal", allow_prefix=True)
"""Pascal, a unit of [pressure][isq.PRESSURE] and [stress][isq.STRESS]."""
J = (N * M).alias("joule", allow_prefix=True)
"""Joule, a unit of [energy][isq.ENERGY], [work][isq.WORK]
 and amount of [heat][isq.HEAT]."""
...
"""Farad, a unit of [capacitance][isq.CAPACITANCE]."""
OHM = (V * A**-1).alias("ohm", allow_prefix=True)
"""Ohm, a unit of electric [resistance][isq.RESISTANCE]."""
SIEMENS = (A * V**-1).alias("siemens", allow_prefix=True)
"""Siemens, a unit of electric [conductance][isq.CONDUCTANCE]."""
WB = (V * S).alias("weber", allow_prefix=True)
"""Weber, a unit of [magnetic flux][isq.MAGNETIC_FLUX]."""
T = (WB * M**-2).alias("tesla", allow_prefix=True)
"""Tesla, a unit of [magnetic flux density][isq.MAGNETIC_FLUX_DENSITY]."""
H = (WB * A**-1).alias("henry", allow_prefix=True)
"""Henry, a unit of [inductance][isq.INDUCTANCE]."""
CELSIUS = Translated(K, Decimal("-273.15"), "celsius")
"""Celsius, a unit of thermodynamic [temperature][isq.TEMPERATURE].
...
"""Lumen, a unit of [luminous flux][isq.LUMINOUS_FLUX]."""
LX = (LM * M**-2).alias("lux", allow_prefix=True)
"""Lux, a unit of [illuminance][isq.ILLUMINANCE]."""
BQ = (S**-1)["activity"].alias("becquerel", allow_prefix=True)
"""Unit of [activity][isq.ACTIVITY] referred to a radionuclide.
...
"""Gray, a unit of [absorbed dose][isq.ABSORBED_DOSE] and [kerma][isq.KERMA]."""
SV = (J * KG**-1)["dose_equivalent"].alias("sievert", allow_prefix=True)
"""Sievert, a unit of [dose equivalent][isq.DOSE_EQUIVALENT]."""
...
"""Planck constant, ([CODATA 2022][isq._citations.CODATA2022])."""
...
"""Elementary charge, ([CODATA 2022][isq._citations.CODATA2022])."""
CONST_PERMEABILITY_VACUUM: Annotated[Decimal, H * M**-1, StdUncertainty(20)] = (
    Decimal("1.25663706127e-6")
)
"""Permeability of free space, ([CODATA 2022][isq._citations.CODATA2022])."""
...
"""Permittivity of free space, ([CODATA 2022][isq._citations.CODATA2022])."""
CONST_BOLTZMANN: Annotated[Decimal, J * K**-1] = Decimal("1.380649e-23")
"""Boltzmann constant, ([CODATA 2022][isq._citations.CODATA2022])."""
CONST_AVOGADRO: Annotated[Decimal, MOL**-1] = Decimal("6.02214076e23")
"""Avogadro constant, ([CODATA 2022][isq._citations.CODATA2022])."""
...
"""Minute, a unit of [time][isq.TIME]."""
HOUR = (60 * MIN).alias("hour")
"""Hour, a unit of [time][isq.TIME]."""
DAY = (24 * HOUR).alias("day")
"""Day, a unit of [time][isq.TIME]."""
...
"""Light-year, a unit of astronomical [length][isq.LENGTH], defined as the
[distance][isq.DISTANCE] that light travels in the vacuum in one year."""

# plane and phase angle
DEG = (LazyProduct((_PI, (180, -1))) * RAD).alias("degree")
"""Degrees (°), a unit of plane [angle][isq.ANGLE]."""
MIN_ANGLE = (Fraction(1, 60) * DEG).alias("minute_angle")
"""Minutes (′), a unit of plane [angle][isq.ANGLE]."""
SEC_ANGLE = (Fraction(1, 60) * MIN_ANGLE).alias("second_angle")
"""Seconds (″) or arcseconds in astronomy, a unit of plane [angle][isq.ANGLE]."""
REV = (LazyProduct((2, _PI)) * RAD).alias("revolution")
"""Revolutions, a unit of plane [angle][isq.ANGLE]."""

# area
SQ_M = M**2
ARE = (100 * SQ_M).alias("are")
HECTARE = (100 * ARE).alias("hectare")
"""Hectare, a unit of land [area][isq.AREA], as adopted by the CIPM in 1879."""

# volume
CU_M = M**3
L = (Fraction(1, 10**3) * CU_M).alias("liter", allow_prefix=True)
"""Liter, a unit of [volume][isq.VOLUME], as adopted by the 16th CGPM in 1979."""

# mass
TONNE = (1_000 * KG).alias("tonne", allow_prefix=True)
"""Tonne, a unit of [mass][isq.MASS], also known as the `metric ton` in the U.S."""
...
"""Electronvolt, the kinetic [energy][isq.ENERGY] acquired by an electron in
passing through a  potential difference of 1 [volt][isq.V] in vacuum."""
...
"""Decibel, [voltage][isq.VOLTAGE] relative to 1 volt, regardless of impedance."""
...
"""Ångström, a unit of [length][isq.LENGTH]."""
BARN = (Fraction(1, 10**28) * SQ_M).alias("barn", allow_prefix=True)
"""Barn, a unit of [area][isq.AREA] for nuclear cross sections."""
BAR = (10**5 * PA).alias("bar", allow_prefix=True)
"""Bar, a unit of [pressure][isq.PRESSURE]."""

MMHG = (
    LazyProduct((CONST_DENSITY_HG, CONST_STANDARD_GRAVITY)) * (MILLI * M)
).alias("millimeter_of_hg")
"""Millimeter of mercury, a unit of [pressure][isq.PRESSURE]."""
MMH2O = (
    LazyProduct((CONST_DENSITY_H2O, CONST_STANDARD_GRAVITY)) * (MILLI * M)
).alias("millimeter_of_h2o")
"""Millimeter of water (conventional), a unit of [pressure][isq.PRESSURE]."""  # [H44 C-59, footnote 12]
...
"""Standard atmosphere, a unit of [pressure][isq.PRESSURE]."""
TORR = (Fraction(1, 760) * ATM).alias("torr")
"""Torr, a unit of [pressure][isq.PRESSURE]."""
KGF = (CONST_STANDARD_GRAVITY * KG * M_PERS2).alias("kg_force")
"""Kilogram-force."""
KWH = ((KILO * W) * HOUR).alias("kilowatt_hour")
"""Kilowatt-hour, commonly used as a billing unit for electric [energy][isq.ENERGY]."""
...
"""Volt-ampere, a unit of [apparent power][isq.APPARENT_POWER]."""
VAR = (V * A).alias("var", allow_prefix=True)
"""Volt-ampere reactive, a unit of [reactive power][isq.REACTIVE_POWER]."""
...
"""[Distance][isq.DISTANCE] between the lowest and highest end of an object.
...
datum.". Specify the particular datum using the [`isq.OriginAt`][] tag.
"""
DEPTH = LENGTH["depth"]
...
sphere; its [length][isq.LENGTH]."""
RADIUS = LENGTH["radius"]
"""Segment in a circle or sphere from its center to its perimeter or surface,
and its [length][isq.LENGTH]."""
ARC_LENGTH = LENGTH["arc_length"]
"""The [distance][isq.DISTANCE] between two points along a section of a curve."""
DISTANCE = LENGTH["distance"]
"""[Length][isq.LENGTH] of the straight line that connects two points in a
measurable space or in an observable physical space."""
RADIAL_DISTANCE = DISTANCE["radial"]
"""The radial [distance][isq.DISTANCE] within a closed non-intersecting
curve/surface. Use [`isq.OriginAt`][] to specify the origin."""
POSITION = QtyKind(M, ("position", VECTOR))
"""Vector representing the position of a point with respect to a given origin
and axes. Specify the origin with the [`isq.OriginAt`][] tag and the coordinate
system (e.g. [`isq.CARTESIAN`][])."""
INITIAL_POSITION = POSITION["initial"]
FINAL_POSITION = POSITION["final"]
DISPLACEMENT = QtyKind(M, ("displacement", VECTOR))
"""Vector that is the shortest [distance][isq.DISTANCE] from the
[initial][isq.INITIAL_POSITION] to the [final position][isq.FINAL_POSITION] of a
point P."""
...
"""[Radius][isq.RADIUS] of a circle which best approximates a curve at a given
point."""
...
[angular displacement][isq.ANGULAR_DISPLACEMENT_CCW]."""
...
"""[Angular velocity][isq.ANGULAR_VELOCITY_CCW], but positive clockwise."""
...
"""[Angular acceleration][isq.ANGULAR_ACCELERATION_CCW], but positive clockwise."""

FREQUENCY = QtyKind(HZ)
"""Number of occurrences or cycles per [time][isq.TIME]."""
...
[acceleration][isq.ACCELERATION]."""
WEIGHT = FORCE["weight"]
FRICTION = FORCE["friction"]
STATIC_FRICTION = FRICTION["static"]
"""Subconcept of friction."""
KINETIC_FRICTION = FRICTION["kinetic"]
"""[Force][isq.FORCE] opposing the motion of a body sliding on a surface."""
ROLLING_FRICTION = FRICTION["rolling"]
"""[Force][isq.FORCE] resisting the motion when a body (such as a ball, tire, or
wheel) rolls on a surface."""
DRAG = FORCE["drag"]
"""Retarding [force][isq.FORCE] on a body moving in a fluid."""

NORMAL_FORCE = FORCE["normal"]
"""The component of a contact [force][isq.FORCE] that is perpendicular to the
surface that an object contacts."""
TANGENTIAL_FORCE = FORCE["tangential"]
"""The component of a contact [force][isq.FORCE] that is parallel to the
surface that an object contacts."""
...
"""Tendency of a [force][isq.FORCE] to rotate an object; counterpart of force in
rotational systems."""
ANGULAR_IMPULSE = QtyKind(N * M * S, (VECTOR,))
PRESSURE = QtyKind(PA)
"""The [force][isq.FORCE] applied perpendicular to the surface of an object per
...
r"""Property of an [area][isq.AREA] reflecting how its points are distributed
...
of [work][isq.WORK] and in the form of [heat][isq.HEAT] and light."""
POWER = QtyKind(W)
POTENTIAL_ENERGY = ENERGY["potential"]
"""[Energy][isq.ENERGY] held by an object because of its
[position][isq.POSITION] relative to other objects or stresses within itself,
rather than its [velocity][isq.VELOCITY]."""
KINETIC_ENERGY = ENERGY["kinetic"]
"""[Energy][isq.ENERGY] of a moving physical body."""
...
WORK = QtyKind(J, ("work",))
"""[Energy][isq.ENERGY] transferred to an object via the application of
[force][isq.FORCE] on it through a [displacement][isq.DISPLACEMENT]."""
...
"""Negative relative change of [volume][isq.VOLUME] per change of
[pressure][isq.PRESSURE] at constant [temperature][isq.TEMPERATURE]."""
ISENTROPIC_COMPRESSIBILITY = COMPRESSIBILITY["isentropic"]
"""Negative relative change of [volume][isq.VOLUME] per change of
[pressure][isq.PRESSURE] at constant [entropy][isq.ENTROPY]."""
WORK_BY_SYSTEM = WORK["by_system"]
"""Work done by the system."""
WORK_ON_SYSTEM = WORK["on_system"]
"""Work done on the system."""
HEAT = QtyKind(J, ("heat",))
"""[Energy][isq.ENERGY] that is transferred from one body to another as the
result of a difference in [temperature][isq.TEMPERATURE]."""
...
"""Measure of [heat][isq.HEAT] transfer on a surface."""
THERMAL_INSULANCE = QtyKind(M**2 * K * W**-1)
THERMAL_RESISTANCE = QtyKind(K * W**-1)
"""Objects' resistance to [heat][isq.HEAT] transfer; reciprocal of
[thermal conductance][isq.THERMAL_CONDUCTANCE]."""
THERMAL_CONDUCTANCE = QtyKind(W * K**-1)
"""Objects' ability to transfer [heat][isq.HEAT]; reciprocal of
[thermal resistance][isq.THERMAL_RESISTANCE]."""
THERMAL_DIFFUSIVITY = QtyKind(M**2 * S**-1)
"""Physical quantity that measures the rate of transfer of [heat][isq.HEAT] of a
material from the hot side to the cold side."""
HEAT_CAPACITY = QtyKind(J * K**-1, ("heat_capacity",))
"""Thermal property describing the [energy][isq.ENERGY] required to change a
material's [temperature][isq.TEMPERATURE]."""
...
"""The negative of the relative [pressure][isq.PRESSURE] change per relative
[volume][isq.VOLUME] change at constant [entropy][isq.ENTROPY]; for an ideal gas
...
[heat transferred to the system][isq.HEAT_TO_SYSTEM] minus the
[work done by the system][isq.WORK_BY_SYSTEM] (closed system, no chemical
reactions)."""
ENTHALPY = ENERGY["enthalpy"]
"""Measure of [energy][isq.ENERGY] in a thermodynamic system; thermodynamic
quantity equivalent to the total [heat][isq.HEAT] content of a system."""
...
[work][isq.WORK] in certain systems."""
ACTIVATION_ENERGY = ENERGY["activation"]
SPECIFIC_ENERGY = QtyKind(J * KG**-1, ("specific_energy",))
"""Physical quantity representing [energy][isq.ENERGY] content per unit
[mass][isq.MASS]."""
...
"""Hypothetical [pressure][isq.PRESSURE] of gas if it alone occupied the
[volume][isq.VOLUME] of the mixture at the same [temperature][isq.TEMPERATURE]."""
...
r"""Ratio of the [partial pressure][isq.PARTIAL_PRESSURE] of water vapor in
humid air to the equilibrium vapor pressure of water at a given
[temperature][isq.TEMPERATURE]."""
RELATIVE_MASS_CONCENTRATION_VAPOUR = Dimensionless(
    "relative_mass_concentration_vapour"
)
"""Quotient of the [mass concentration][isq.MASS_CONCENTRATION] of water vapor
...
[temperature][isq.TEMPERATURE]. Approximation to
[relative humidity][isq.RELATIVE_HUMIDITY]."""
DEW_POINT = TEMPERATURE["dew_point"]
"""The [temperature][isq.TEMPERATURE] at which air becomes saturated with water
vapour."""
#
...
[potential difference][isq.ELECTRIC_POTENTIAL_DIFFERENCE] between two points."""
...
"""Surface integral of the electric [flux density][isq.ELECTRIC_FLUX_DENSITY];
...
"""Integral of the [magnetic vector potential][isq.MAGNETIC_VECTOR_POTENTIAL]
along a path."""
...
"""Highest value of [magnetic flux][isq.MAGNETIC_FLUX] produced by a current
loop in circuit theory.
The definition is consistent with the more general definition of
[linked flux][isq.LINKED_MAGNETIC_FLUX].
"""
...
"""Measure of directional [energy][isq.ENERGY] flux."""
...
[magnetic field strength][isq.MAGNETIC_FIELD_STRENGTH]."""
...
"""In physics, the ratio of [magnetomotive force][isq.MAGNETOMOTIVE_FORCE] to
[magnetic flux][isq.MAGNETIC_FLUX]; the magnetic analogue of electrical
resistance."""
PERMEANCE = QtyKind(H, ("permeance",))
INDUCTANCE = QtyKind(H, ("inductance",))
"""Property of electrical conductors to oppose changes in [current][isq.CURRENT]
flow."""
...
material allows the flow of electric [current][isq.CURRENT]."""
RESISTIVITY = QtyKind(OHM * M)
INSTANTANEOUS_POWER = POWER["instantaneous"]
# TODO: average
RESISTANCE = QtyKind(OHM)
"""Opposition to the passage of an electric [current][isq.CURRENT]."""
...
"""Complex representation of an oscillating electric [current][isq.CURRENT]."""
VOLTAGE_PHASOR = VOLTAGE["phasor", COMPLEX]
"""Complex representation of an oscillating [voltage][isq.VOLTAGE]."""
...
"""Real part of the complex [impedance][isq.IMPEDANCE]."""
REACTANCE = QtyKind(OHM, ("reactance",))
"""A circuit element's opposition to changes in electric [current][isq.CURRENT]
due to its [inductance][isq.INDUCTANCE] or [capacitance][isq.CAPACITANCE]."""
...
"""Real part of the complex [admittance][isq.ADMITTANCE]."""
SUSCEPTANCE = QtyKind(SIEMENS, ("susceptance",))
"""Imaginary part of the [admittance][isq.ADMITTANCE]."""
...
"""Product of RMS [voltage][isq.VOLTAGE] and RMS [current][isq.CURRENT] in an AC
electrical system."""
...
"""[Energy][isq.ENERGY] propagated by electromagnetic waves."""
...
"""[Power][isq.POWER] carried by electromagnetic waves."""
...
"""Areal density of [radiant intensity][isq.RADIANT_INTENSITY] in a given
direction."""
SPECTRAL_RADIANCE = QtyKind(W * SR**-1 * M**-3)
"""[Radiance][isq.RADIANCE] of a surface."""
...
"""Specify the [photometric condition][isq.PhotometricCondition] with
[tags][isq.Tagged]."""
...
"""Photometric measure of the [luminous intensity][isq.LUMINOUS_INTENSITY] per
[area][isq.AREA] of light travelling in a given direction."""
...
"""[Energy][isq.ENERGY] carried by a single photon."""
PHOTON_FLUX = QtyKind(S**-1, ("photon_flux",))
PHOTON_INTENSITY = QtyKind(S**-1 * SR**-1)
PHOTON_RADIANCE = QtyKind(M**-2 * S**-1 * SR**-1)
"""[Area][isq.AREA] density of the [photon intensity][isq.PHOTON_INTENSITY] in a
specified direction."""
...
"""[Emissivity][isq.EMISSIVITY] as a function of [wavelength][isq.WAVELENGTH]."""
...
"""Local [pressure][isq.PRESSURE] deviation from the ambient atmospheric
...
"""Instantaneous [displacement][isq.DISPLACEMENT] of a particle from its
equilibrium [position][isq.POSITION] in a medium as it transmits a sound
wave."""
...
"""[Time][isq.TIME] after which the
[sound energy density][isq.SOUND_ENERGY_DENSITY] has fallen to a certain
...
"""The particular molecule should be specified with [tags][isq.Tagged]."""
...
"""Number of molecules of a substance in a mixture per [volume][isq.VOLUME]."""
...
species, in terms of [amount of substance][isq.AMOUNT_OF_SUBSTANCE] in a given
[volume][isq.VOLUME]; most commonly expressed in units of moles of solute per
litre of solution."""
...
"""The chosen value of molality, commonly 1 [mol][isq.MOL] per [kg][isq.KG]."""
LATENT_HEAT_OF_PHASE_TRANSITION = LATENT_HEAT["phase_transition"]
"""[Energy][isq.ENERGY] to be added to or removed from a system under constant
[temperature][isq.TEMPERATURE] and [pressure][isq.PRESSURE] to undergo a
...
[absolute activity][isq.ABSOLUTE_ACTIVITY] of the pure substance at the same
[temperature][isq.TEMPERATURE] but at standard pressure (10⁵ Pa)."""
...
[Boltzmann constant][isq.CONST_BOLTZMANN]."""
# specific gas constant defined in ISO 80000-5
MEAN_FREE_PATH = DISTANCE["mean_free_path"]
"""Average [distance][isq.DISTANCE] travelled by a moving particle between
successive impacts."""
...
# these are not in the iso standard
...
REST_MASS = MASS["rest"]
"""[Mass][isq.MASS] of a particle at rest in an intertial frame."""
REST_ENERGY = ENERGY["rest"]
ATOMIC_MASS = MASS["atomic"]
"""[Rest mass][isq.REST_MASS] of an atom in its ground state."""
NUCLIDIC_MASS = MASS["nuclidic"]
"""[Mass][isq.MASS] of a nuclide in its ground state."""
UNIFIED_ATOMIC_MASS_CONSTANT = MASS["unified_atomic_mass_constant"]
"""A twelfth of the [mass][isq.MASS] of a carbon-12 atom in its ground state."""
CHARGE_NUMBER = Dimensionless("charge_number")
BOHR_RADIUS = LENGTH["bohr_radius"]
"""Physical constant; the most probable [distance][isq.DISTANCE] between an
...
"""Atomic unit of [energy][isq.ENERGY]."""
ATOMIC_MAGNETIC_DIPOLE_MOMENT = MAGNETIC_MOMENT["atomic"]
BOHR_MAGNETON = QtyKind(A * M**2, ("bohr_magneton",))
"""Unit of [magnetic moment][isq.MAGNETIC_MOMENT] (approx. 9.2 J/T); the
[magnetic dipole moment][isq.ATOMIC_MAGNETIC_DIPOLE_MOMENT] of an electron
orbiting an atom with [angular momentum][isq.ANGULAR_MOMENTUM] ℏ in the Bohr
model."""
NUCLEAR_MAGNETON = QtyKind(A * M**2, ("nuclear_magneton",))
"""Physical constant of [magnetic moment][isq.MAGNETIC_MOMENT]."""
SPIN = QtyKind(J * S, ("spin", VECTOR))
"""Intrinsic form of [angular momentum][isq.ANGULAR_MOMENTUM] as a property of
quantum particles."""
...
[angular momentum][isq.ANGULAR_MOMENTUM] and describes the shape of the orbital,
...
[total angular momentum][isq.TOTAL_ANGULAR_MOMENTUM] of an atom."""
...
"""g-factor for electron with [spin][isq.SPIN] and orbital
[angular momentum][isq.ANGULAR_MOMENTUM]."""
...
"""Angular [frequency][isq.FREQUENCY] of a charged particle moving on a circular
path perpendicular to a uniform magnetic field."""
GYRORADIUS = RADIUS["gyroradius"]
"""[Radius][isq.RADIUS] of the circular movement of an electrically charged
particle in a magnetic field."""
NUCLEAR_QUADRUPOLE_MOMENT = QtyKind(M**2, ("nuclear_quadrupole_moment",))
"""Measure for the deviation of the nuclear [charge density][isq.CHARGE_DENSITY]
from spherical symmetry."""
NUCLEAR_RADIUS = RADIUS["nuclear"]
"""Measure of the size of atomic nuclei."""
ELECTRON_RADIUS = RADIUS["electron"]
"""Physical constant providing [length][isq.LENGTH] scale to interatomic
interactions."""
COMPTON_WAVELENGTH = WAVELENGTH["compton"]
"""In quantum mechanics, the [wavelength][isq.WAVELENGTH] of a photon whose
[energy][isq.ENERGY] is the same as the [rest energy][isq.REST_ENERGY] of a
particle."""
MASS_EXCESS = MASS["excess"]
MASS_DEFECT = MASS["defect"]
"""Equivalent [mass][isq.MASS] of the binding energy of an atomic nucleus."""
...
"""In nuclear physics, mean [duration][isq.DURATION] after which half of the
...
[energy][isq.ENERGY] of an incident particle in the reference frame of the
target particle."""
...
"""In nuclear physics, mean [energy][isq.ENERGY] of emitted, transferred or
received particles."""
...
"""[Thickness][isq.THICKNESS] of an attenuating layer which reduces the value of
...
"""Mean path [length][isq.LENGTH] traveled by particles of a given initial
[energy][isq.ENERGY] slowing down to rest in a given material."""
MEAN_MASS_RANGE = QtyKind(KG * M**-2, ("mean_mass_range",))
LINEAR_IONIZATION = QtyKind(M**-1, ("linear_ionization",))
"""Mean number of elementary charges per path [length][isq.LENGTH] of all ions
...
"""Number of ions per [volume][isq.VOLUME]."""
...
"""Natural logarithm of the quotient of a reference [energy][isq.ENERGY] and the
kinetic energy of a neutron."""
AVERAGE_LOGARITHMIC_ENERGY_DECREMENT = Dimensionless(
    "average_logarithmic_energy_decrement"
)
"""Average increase in [lethargy][isq.LETHARGY] per collision of neutrons with
atomic nuclei."""
MEAN_FREE_PATH_ATOMIC = MEAN_FREE_PATH["atomic"]
"""In nuclear physics, average [distance][isq.DISTANCE] that particles travel
between two specified interactions."""
SLOWING_DOWN_AREA = AREA["slowing_down"]
"""One sixth of the mean squared [distance][isq.DISTANCE] between a neutron
source and the point at which the neutron reaches a given [energy][isq.ENERGY]."""
DIFFUSION_AREA = AREA["diffusion"]
"""One sixth of the mean squared [distance][isq.DISTANCE] where a neutron enters
an [energy][isq.ENERGY] class and the point where it leaves this class."""
MIGRATION_AREA = AREA["migration"]
"""Sum of the [slowing-down area][isq.SLOWING_DOWN_AREA] of neutrons from
fission to thermal [energy][isq.ENERGY] and the
[diffusion area][isq.DIFFUSION_AREA] of thermal neutrons."""
...
"""In nuclear physics, the [multiplication factor][isq.MULTIPLICATION_FACTOR]
for an infinite medium."""
REACTOR_TIME_CONSTANT = DURATION["reactor_time_constant"]
"""[Duration][isq.DURATION], in which the neutron
[fluence rate][isq.PARTICLE_FLUENCE_RATE] in a reactor changes by a factor e."""
ENERGY_IMPARTED = ENERGY["imparted"]
MEAN_ENERGY_IMPARTED = ENERGY["mean_imparted"]
"""In nuclear physics, expectation value of the
[energy imparted][isq.ENERGY_IMPARTED]."""
...
[dose equivalent][isq.DOSE_EQUIVALENT]."""
...
"""Kinetic [energy][isq.ENERGY] released per [mass][isq.MASS]."""
...
"""For a body moving in a fluid the quotient of [drag][isq.DRAG] and
gravitational [force][isq.FORCE]."""
...
"""Coefficient that relates the [lift][isq.LIFT] generated by a lifting body to
other parameters."""
...
"""Dimensionless [pressure drop][isq.PRESSURE_DROP] along a channel."""
...
"""Ratio of inertial [force][isq.FORCE] to Coriolis force."""
...
"""Characteristic number for the [pressure drop][isq.PRESSURE_DROP] in a pipe
...
"""Nondimensional number showing the ratio of inertial [force][isq.FORCE] to
viscous force."""
...
"""Characteristic number of [heat][isq.HEAT] transport in fluids."""
...
"""Characteristic number for [heat][isq.HEAT] transfer by conduction into a
body."""
STANTON_NUMBER = Dimensionless("stanton_number")
J_FACTOR_HEAT_TRANSFER = Dimensionless("j_factor_heat_transfer")
"""Characteristic number for the relation between [heat][isq.HEAT] and
[mass][isq.MASS] transfer in a fluid."""
...
"""In thermodynamics, the ratio of [heat][isq.HEAT] transfer irreversibility to
total irreversibility."""
STEFAN_NUMBER = Dimensionless("stefan_number")
"""Characteristic number for the relation between [heat][isq.HEAT] and
[latent heat][isq.LATENT_HEAT] of a binary mixture undergoing a phase
transition."""
BRINKMAN_NUMBER = Dimensionless("brinkman_number")
"""Characteristic number of a fluid for the relation between [heat][isq.HEAT]
...
[energy][isq.ENERGY] transfer by momentum and by thermal conduction."""
...
"""Characteristic number for the relation between [heat][isq.HEAT] generation
and conduction in a body."""
BOLTZMANN_NUMBER = Dimensionless("boltzmann_number")
"""Characteristic number for the relation between convective [heat][isq.HEAT] and
...
"""Characteristic number for [mass][isq.MASS] transfer at the boundary of a
fluid."""
...
"""Characteristic number for the relation between [mass][isq.MASS] transport
...
"""Characteristic number for the relation between [mass][isq.MASS] transfer rate
...
"""Used to determine the motion of fluids due to [density][isq.DENSITY]
differences."""
EXPANSION_NUMBER = Dimensionless("expansion_number")
"""Characteristic number for the relation of buoyancy and internal
[force][isq.FORCE] for gas bubbles rising in a liquid."""
...
"""Ratio of viscous drag forces to [surface tension][isq.SURFACE_TENSION] in
fluids."""
...
[surface tension][isq.SURFACE_TENSION] forces."""
...
"""Characteristic number for the relation between plasma [speed][isq.SPEED] and
Alfvén wave speed."""
...
"""Quotient of gas and magnetic [pressure][isq.PRESSURE] in a gas or plasma."""
...
Lorentz [force][isq.FORCE] to the viscosity."""
...
"""Quotient of magnetic and inertial [force][isq.FORCE] in an electrically
conducting fluid."""
...
"""Quotient of Coulomb and Lorentz [force][isq.FORCE] on moving, electrically
charged particles."""
HALL_NUMBER = Dimensionless("hall_number")
"""Quotient of gyro and collision [frequency][isq.FREQUENCY] in a plasma."""
LUNDQUIST_NUMBER = Dimensionless("lundquist_number")
"""Quotient of Alfvén and magneto-dynamic [speed][isq.SPEED] in a plasma."""
JOULE_MAGNETIC_NUMBER = Dimensionless("joule_magnetic_number")
"""Quotient of Joule [heat][isq.HEAT] and magnetic field [energy][isq.ENERGY] in
a plasma."""
GRASHOF_MAGNETIC_NUMBER = Dimensionless("grashof_magnetic_number")
"""Characteristic number for the [heat][isq.HEAT] transfer by thermo-magnetic
convection of a paramagnetic fluid under the influence of gravity."""
NAZE_NUMBER = Dimensionless("naze_number")
"""Quotient of Alfvén wave [speed][isq.SPEED] and
[sound speed][isq.SPEED_OF_SOUND] in a plasma."""
REYNOLDS_ELECTRIC_NUMBER = Dimensionless("reynolds_electric_number")
"""Quotient of the [speed][isq.SPEED] of an electrically conducting fluid and
...
[current][isq.CURRENT] and [magnetic field strength][isq.MAGNETIC_FIELD_STRENGTH]
...
[fundamental lattice vector][isq.FUNDAMENTAL_LATTICE_VECTORS] is an integral
multiple of two Pi."""
...
"""[Distance][isq.DISTANCE] between adjacent lattice planes."""
BRAGG_ANGLE = ANGLE["bragg"]
"""In X-ray crystallography, [angle][isq.ANGLE] between lattice plane and
scattered ray."""
...
"""[Position][isq.POSITION] vector of a particle."""
EQUILIBRIUM_POSITION_VECTOR = POSITION["equilibrium"]
"""In condensed matter physics, [position][isq.POSITION] vector of an atom or
ion in equilibrium."""
...
"""Angular [wavenumber][isq.WAVENUMBER] of an electron in a state on the Fermi
surface."""
...
"""Describes the effect that changing the [volume][isq.VOLUME] of a crystal
...
"""[Voltage][isq.VOLTAGE] caused by the thermoelectric effect."""
...
"""Minimum amount of [energy][isq.ENERGY] required to remove an electron from an
atom or molecule in the gaseous state."""
ELECTRON_AFFINITY = ENERGY["electron_affinity"]
"""In condensed matter physics, [energy][isq.ENERGY] difference between an
...
"""Concept in quantum mechanics referring to the [energy][isq.ENERGY] difference
...
"""Smallest [energy][isq.ENERGY] difference between neighboring conduction
...
"""Number of donor levels per [volume][isq.VOLUME]."""
ACCEPTOR_DENSITY = NUMBER_DENSITY["acceptor"]
"""Number of acceptor levels per [volume][isq.VOLUME]."""
EFFECTIVE_MASS = MASS["effective"]
"""The [mass][isq.MASS] that it seems to have when responding to forces, or the
...
"""In condensed matter physics, [time constant][isq.TIME_CONSTANT] for
...
"""Average [time][isq.TIME] taken for free semiconductor electrons or holes to
...
"""Width of the forbidden [energy][isq.ENERGY] band in a superconductor."""
LONDON_PENETRATION_DEPTH = LENGTH["london_penetration_depth"]
"""[Distance][isq.DISTANCE] to which a magnetic field penetrates into a
superconductor."""
COHERENCE_LENGTH = LENGTH["coherence"]
"""Characteristic [length][isq.LENGTH] in a superconductor."""
...
"""[Time][isq.TIME] average of the [length][isq.LENGTH] of a queue."""
...
"""Completed calls per [time][isq.TIME]."""
...
"""[Frequency][isq.FREQUENCY] at which CPU chip or core is operating."""
