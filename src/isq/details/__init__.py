"""[Dimensionless quantities][isq.Dimensionless] and
[quantity kinds][isq.QtyKind] themselves do not store the precise definition
of how it is derived.

This module contains **optional** details that "link" them together. It is
stored in a [dictionary of expressions to its details][isq.details.Details]. A
detail can contain multiple [equations][isq.details.Equation] or [common
symbols][isq.details.Symbol].

Our [mkdocs plugin][isq.mkdocs.plugin] reads dictionaries from this module to
...
[definition's where clause][isq.details.Equation.where]."""
...
class Symbol(HasKaTeXWhere):
    ...
    Since the [equation][isq.details.Equation] class already contains the
