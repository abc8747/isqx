...
function findHighlightTargets(definition) {
    ...
    const targets = Array.from(
        definition.querySelectorAll(".isq-where > .isq-where-row"),
    )
        .map((row, i) => {
            const katex = row.querySelector(".isq-symbol .katex");
...
export function highlightVariables() {
    const definitions = document.querySelectorAll("div.isq-detail");
    ...
    for (const definition of definitions) {
        ...
        for (const target of targets) {
            ...
            const symbolNode = target.row.querySelector(".isq-symbol .katex");
            ...
            for (const el of interactiveGroup) {
                ...
                el.addEventListener("mouseenter", () =>
                    interactiveGroup.forEach((el) =>
                        el.classList.add("isq-hover"),
                    ),
                );
                el.addEventListener("mouseleave", () =>
                    interactiveGroup.forEach((el) =>
                        el.classList.remove("isq-hover"),
                    ),
                );
            }

            for (const node of styledElements) {
                node.classList.add("isq-highlight");
            }
        }
    }
}
