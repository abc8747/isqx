...
const Search: Component<{
  onAddToSelection?: (path: PublicApiPath) => void;
  nodes: GraphNode[];
  onSelect: (path: PublicApiPath) => void;
  onPreview?: (path: PublicApiPath | null) => void;
  ...
  const schedulePreview = (path: PublicApiPath | null) => {
    cancelPreviewTimeout();
  ...
  const commitSelection = (path: PublicApiPath) => {
    cancelPreviewTimeout();
    props.onPreview?.(null);
    props.onSelect(path);
    setQuery("");
  ...
  const addToSelection = (path: PublicApiPath) => {
    cancelPreviewTimeout();
    props.onPreview?.(null);
  ...
  const handleKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.key === "Enter") {
      ...
      if (result) commitSelection(result.publicApiPath);
      return;
    }
              ...
              {(result, index) => (
                <div
                  ...
                  onMouseEnter={() => {
                    ...
                    schedulePreview(result.publicApiPath);
                  }}
                  onMouseLeave={() => {
                    setHoveredIndex(null);
                ...
                >
                  <button
                    ...
                    onClick={() => commitSelection(result.publicApiPath)}
                  >
                    <span class={styles.resultName}>{result.name}</span>
                    <span class={styles.resultPath}>
                      {result.publicApiPath}
                    </span>
                  </button>
                  <button
                    type="button"
                    ...
                    onClick={() => addToSelection(result.publicApiPath)}
                  >
